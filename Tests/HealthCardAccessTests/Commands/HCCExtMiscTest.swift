...
class HCCExtMiscTest: XCTestCase {
    ...
    func testGenerateAsymmetricKeyPair() {
        guard let key = try? Key(8) else {
            Nimble.fail("could not create reference key")
            return
        }
        //swiftlint:disable line_length
        let responseStatus: [UInt16] = [0x9000, 0x6400, 0x6581, 0x6982, 0x6985, 0x6a88, 0x63c0, 0x63c1, 0x63c2, 0x63c3, 0x63c4, 0x63c5, 0x63c6, 0x63c7, 0x63c8, 0x63c9, 0x63ca, 0x63cb, 0x63cc, 0x63cd, 0x63ce, 0x63cf]
        ...
        let testCases: [GAKPTestCase] = [
            (name: "ReadOnly", expected: (Data([0x0, 0x46, 0x81, 0x0, 0x0, 0x0, 0x0]), responseStatus), mode: .readOnly(reference: nil, dfSpecific: false)),
            (name: "ReadOnly with key", expected: (Data([0x0, 0x46, 0x81, 0x8, 0x0, 0x0, 0x0]), responseStatus), mode: .readOnly(reference: key, dfSpecific: false)),
            (name: "ReadOnly with dfSpecific key", expected: (Data([0x0, 0x46, 0x81, 0x88, 0x0, 0x0, 0x0]), responseStatus), mode: .readOnly(reference: key, dfSpecific: true)),
            // Generate out
            (name: "Generate out", expected: (Data([0x0, 0x46, 0x80, 0x0, 0x0, 0x0, 0x0]), responseStatus), mode: .generate(reference: nil, dfSpecific: false, overwrite: false, out: true)),
            (name: "Generate with Key and out", expected: (Data([0x0, 0x46, 0x80, 0x8, 0x0, 0x0, 0x0]), responseStatus), mode: .generate(reference: key, dfSpecific: false, overwrite: false, out: true)),
            (name: "Generate with dfSpecific Key and out", expected: (Data([0x0, 0x46, 0x80, 0x88, 0x0, 0x0, 0x0]), responseStatus), mode: .generate(reference: key, dfSpecific: true, overwrite: false, out: true)),
            // Generate no-out
            (name: "Generate No-out", expected: (Data([0x0, 0x46, 0x84, 0x0]), responseStatus), mode: .generate(reference: nil, dfSpecific: false, overwrite: false, out: false)),
            (name: "Generate with key and No-out", expected: (Data([0x0, 0x46, 0x84, 0x8]), responseStatus), mode: .generate(reference: key, dfSpecific: false, overwrite: false, out: false)),
            (name: "Generate with dfSpecific Key and No-out", expected: (Data([0x0, 0x46, 0x84, 0x88]), responseStatus), mode: .generate(reference: key, dfSpecific: true, overwrite: false, out: false)),
            // Generate overwrite and out
            (name: "Generate overwrite out", expected: (Data([0x0, 0x46, 0xc0, 0x0, 0x0, 0x0, 0x0]), responseStatus), mode: .generate(reference: nil, dfSpecific: false, overwrite: true, out: true)),
            (name: "Generate overwrite with key and out", expected: (Data([0x0, 0x46, 0xc0, 0x8, 0x0, 0x0, 0x0]), responseStatus), mode: .generate(reference: key, dfSpecific: false, overwrite: true, out: true)),
            (name: "Generate overwrite with dfSpecific Key and out", expected: (Data([0x0, 0x46, 0xc0, 0x88, 0x0, 0x0, 0x0]), responseStatus), mode: .generate(reference: key, dfSpecific: true, overwrite: true, out: true)),
            // Generate overwrite and no-out
            (name: "Generate overwrite No-out", expected: (Data([0x0, 0x46, 0xc4, 0x0]), responseStatus), mode: .generate(reference: nil, dfSpecific: false, overwrite: true, out: false)),
            (name: "Generate overwritewith key and No-out", expected: (Data([0x0, 0x46, 0xc4, 0x8]), responseStatus), mode: .generate(reference: key, dfSpecific: false, overwrite: true, out: false)),
            (name: "Generate overwritewith dfSpecific Key and No-out", expected: (Data([0x0, 0x46, 0xc4, 0x88]), responseStatus), mode: .generate(reference: key, dfSpecific: true, overwrite: true, out: false))
        ]
