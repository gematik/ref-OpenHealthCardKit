...
extension CardType {
    ...
    public func openSecureSessionAsync(
        ...
        // Read/Determine ApplicationIdentifier of the card's initial application
        let cardAid = try await channel.determineCardAidAsync(writeTimeout: writeTimeout, readTimeout: readTimeout)

        // Read EF.CardAccess and determine the algorithm for the key agreement (e.g. PACE)
        let keyAgreementAlgorithm = try await channel.readKeyAgreementAlgorithmAsync(cardAid: cardAid)
        // Read EF.Version2 and determine HealthCardPropertyType
        let cardType = try await channel.readCardTypeAsync(
            cardAid: cardAid,
            writeTimeout: writeTimeout,
            readTimeout: readTimeout
        )
        let healthCard = try HealthCard(card: self, status: .valid(cardType: cardType))
        let sessionKey = try await keyAgreementAlgorithm.negotiateSessionKeyAsync(
            card: healthCard,
            can: can,
            writeTimeout: writeTimeout,
            readTimeout: readTimeout
