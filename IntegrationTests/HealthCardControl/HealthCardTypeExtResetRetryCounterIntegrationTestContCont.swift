...
final class HealthCardTypeExtResetRetryCounterIntegrationTestContCont: CardSimulationTerminalTestCase {
    ...
    func testResetRetryCounterEgk21_wrongSecretWarning() async throws {
        ...
        expect(response) == ResetRetryCounterResponse.wrongSecretWarning(retryCount: 8)

        // Without setting a new PIN
        response = try await Self.healthCard.resetRetryCounter(
            puk: wrongPuk,
            type: EgkFileSystem.Pin.mrpinHome,
            dfSpecific: false
        )
        expect(response) == ResetRetryCounterResponse.wrongSecretWarning(retryCount: 7)
        ...
        expect(response) == ResetRetryCounterResponse.wrongSecretWarning(retryCount: 6)
    }
}
